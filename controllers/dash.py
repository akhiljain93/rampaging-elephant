...
def t_topics():
    ...
    if not filt: # if filter is not applied
        latest = db(table.id>0).select(table.time.max().with_alias('time'),limitby=(0,1)).first().time - 3600*1000
        day1 = db(table.time==latest).select(table.topic.with_alias('topic'),table.time.with_alias('time'),table.volume.sum().with_alias('volumesum'), groupby=str(table)+'.topic,'+str(table)+'.time')
        dayhist = db(table.time>(latest-4*60*60*24*1000))(table.time<latest).select(table.topic.with_alias('topic'),table.time.with_alias('time'),table.volume.sum().with_alias('volumesum'), groupby=str(table)+'.topic,'+str(table)+'.time', orderby=table.time)
    elif cluswise: # if cluster id is provided
        latest = db(table.cledge==db.cledges.id)((db.cledges.cluster1==filt)|(db.cledges.cluster2 == filt)).select(table.time.max().with_alias('time'),limitby=(0,1)).first().time - 3600*1000
        day1 = db(table.time==latest)(table.cledge==db.cledges.id)((db.cledges.cluster1==filt)|(db.cledges.cluster2 == filt)).select(table.topic.with_alias('topic'),table.time.with_alias('time'),table.volume.sum().with_alias('volumesum'), groupby=str(table)+'.topic,'+str(table)+'.time')
        dayhist = db(table.time>(latest-4*60*60*24*1000))(table.time<latest)(table.cledge==db.cledges.id)((db.cledges.cluster1==filt)|(db.cledges.cluster2 == filt)).select(table.topic.with_alias('topic'),table.time.with_alias('time'),table.volume.sum().with_alias('volumesum'), groupby=str(table)+'.topic,'+str(table)+'.time')
    else: # if location is provided
        latest = db(table.locedge==db.locedges.id)((db.locedges.loc1==filt)|(db.locedges.loc2==filt)).select(table.time.max().with_alias('time'),limitby=(0,1)).first().time - 3600*1000
        day1 = db(table.time==latest)(table.locedge==db.locedges.id)((db.locedges.loc1==filt)|(db.locedges.loc2==filt)).select(table.topic.with_alias('topic'),table.time.with_alias('time'),table.volume.sum().with_alias('volumesum'), groupby=str(table)+'.topic,'+str(table)+'.time')
        dayhist = db(table.time>(latest-4*60*60*24*1000))(table.time<latest)(table.locedge==db.locedges.id)((db.locedges.loc1==filt)|(db.locedges.loc2==filt)).select(table.topic.with_alias('topic'),table.time.with_alias('time'),table.volume.sum().with_alias('volumesum'), groupby=str(table)+'.topic,'+str(table)+'.time')
    score = []
    for x in day1:
        arr = []
        ...
        for y in dayhist:
            if (x.topic==y.topic):
                for z in range((y.time - currentdate)/(60*60*24*1000)):
                    arr.append(0)
                arr.append(y.volumesum)
                currentdate = y.time + 60*60*24*1000
        if len(arr) > 0:
            score.append({'topic':x.topic, 'score':int(zscore(x.volumesum,arr)*100)/100.0})
    score.sort(sortthis)
    score = map(lambda x: dict(topic=x['topic'],score=(x['score'])),score)
    return dict(abeo=score, latest = latest)
